...
def lessons_detail(request, slug):
    ...
    if request.method == "POST":
        ...
        if comment_form.is_valid():
            ...
            if CommentOnLesson.objects.filter(
                ...
                messages.error(
                    request, "You have already commented on this lesson date."
                )
                return redirect("lessons_detail", slug=slug)
    ...
    comments = CommentOnLesson.objects.filter(lesson_type=lesson).order_by(
        "-created_on"
    )
...
def comment_edit(request, slug, comment_id):
    ...
    context = {
        ...
        "lesson": post,
        "comment": comment,
        "selected_lesson_date_id": (
            comment.lesson_date.id if comment.lesson_date else None
        ),
...
def timeslots_for_date(request, date):
    ...
    booked_slots = LessonDate.objects.filter(date=date_obj).values_list(
        "start_time", "end_time"
    )
    ...
    for slot in time_slots:
        ...
        slots_with_status.append(
            {"time": slot.strftime("%H:%M"), "is_booked": is_booked}
        )
...
def booking_form(request, date, slot):
    ...
    try:
        ...
        end_time = (
            datetime.combine(datetime.today(), slot_time) + timedelta(hours=1)
        ).time()
    ...
    lesson_date = LessonDate.objects.filter(
        date=date, start_time=slot_time
    ).first()
    ...
    if request.method == "POST":
        ...
        print(
            f"Selected lesson_id: {lesson_id}, instructor_id: {instructor_id}"
        )
        ...
        try:
            ...
            messages.error(
                request, "Selected lesson or instructor does not exist."
            )
